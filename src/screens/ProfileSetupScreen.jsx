...
export const ProfileSetupScreen = ({ onSave, currentProfile }) => {
    ...
    return (
        <div style={styles.profileContainer}>
            ...
            <div style={styles.inputGroup}>
                ...
                <div style={styles.optionContainer}>
                    {['None', 'Vegetarian', 'Vegan', 'Gluten-Free'].map(item => (
                        <button key={item} style={profile.diet === item ? styles.optionButtonSelected : styles.optionButton} onClick={() => updateProfile('diet', item)}>{item}</button>
                    ))}
                </div>
            </div>
            <div style={styles.inputGroup}>
                ...
                <div style={styles.optionContainer}>
                    {['Italian', 'Mexican', 'Japanese', 'American', 'Chinese', 'Indian', 'Thai', 'Vietnamese', 'French', 'Mediterranean', 'Korean'].map(item => (
                        <button key={item} style={profile.cuisines.includes(item) ? styles.optionButtonSelected : styles.optionButton} onClick={() => handleCuisineToggle(item)}>{item}</button>
                    ))}
                </div>
            </div>
            <div style={styles.inputGroup}>
                <label style={styles.label}>Max Distance (km)</label>
                <p style={styles.sliderValue}>{profile.distance} km</p>
                <div style={styles.optionContainer}>
                   {[1, 5, 10, 20].map(val => (
                       <button key={val} style={profile.distance === val ? styles.optionButtonSelected : styles.optionButton} onClick={() => updateProfile('distance', val)}>{val} km</button>
                   ))}
                </div>
            </div>
            <div style={styles.inputGroup}>
            ...
            <div style={styles.inputGroup}>
                <label style={styles.label}>You are looking to eat</label>
                <div style={styles.optionContainer}>
                    <button style={profile.eatingTime === 'NOW' ? styles.optionButtonSelected : styles.optionButton} onClick={() => updateProfile('eatingTime', 'NOW')}>RIGHT NOW</button>
                    <button style={profile.eatingTime === 'LATER' ? styles.optionButtonSelected : styles.optionButton} onClick={() => updateProfile('eatingTime', 'LATER')}>LATER</button>
                </div>
            </div>
            <button style={styles.saveButton} onClick={() => onSave(profile)}>Save & Find Food</button>
        </div>
    );
};